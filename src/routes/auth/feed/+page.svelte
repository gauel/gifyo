	...
	*/
	$: $scaleImg, (() => {
		if(gifImage && !preventTransform) {
			gsap.to(gifImage, {
				...
				y: 0.5*$pointerMoveY,
				duration: 0,
				borderRadius: `${40-(0.05*$pointerMoveY)}px ${40-(0.05*$pointerMoveY)}px ${24-(0.05*$pointerMoveY)}px ${24-(0.05*$pointerMoveY)}px`,
				ease: superEaseEffect
			})
	...
	const handleTouchMove = (e) => {
		...
		if($pointerMoveY >=150) {
			//releaseToClose = true;
			if(playHaptic) {
				//Haptics.impact({ style: ImpactStyle.Soft });
				playHaptic = false;
			}
		}else{
			releaseToClose = false;
		...
		if($pointerMoveY > 0) {
			...
			$state.moving = true
			$scaleImg = (1-(0.0008*$pointerMoveY))
			scaleGrid = (1.5-(0.0005*$pointerMoveY))
...
<div class="text-white relative max-w-[480px] mx-auto">
	<div class="z-[106] absolute w-full h-[10%] flex items-center justify-start pt-[var(--safe-area-top)] transition-all duration-500 px-4 ease-in-out{$state.opened ? "opacity-0 -translate-y-full" : "opacity-100 translate-y-none"}">
		<Logo style="h-6" />
	</div>
	...
	<div class="grid grid-cols-2 gap-px md:pt-16 {$state.moving ? "transition-none" : "transition-transform"} duration-[500ms] ease-[0.212,0.114,0.242,0.2815]
	{$state.opened ? "pointer-events-none" : "pointer-events-auto"}"
	bind:this={mainGrid}>
		<div
			class="backdrop fixed top-0 left-0 w-full h-full bg-black pointer-events-none z-[1] {$state.moving ? "transition-none" : "transition-opacity"} duration-[250ms]
	...
	>
		...
		<div class="h-[800px] bg-neutral-900 flex-shrink-0 rounded-t-[24px] px-4 pt-4 transition-all duration-[250ms] ease-in 
...
<style lang="css">
	...
	*/
	/*
	:global(.img-wrapper img, #gif-preview img) {
		transition: all 400ms cubic-bezier(0.2, 0.8, 0.2, 1);
	}
	*/
	
	:global(.closed .img-wrapper img) {
		border-radius: 0px !important;
	}

	:global(.opening #gif-preview img) {
